...
WXDLLIMPEXP_NET void (*g_ftp_talkback)(const wxChar*) = NULL;

/*
void wxLogTraceFTP(const wxChar * mask, const wxChar * szFormat, ...)
{
    va_list argptr;
    va_start(argptr, szFormat);
    wxChar str[255];
    wxVsnprintf(str, WXSIZEOF(str), szFormat, argptr);
    wxLogTrace(mask, str);
    //wxVLogTrace(mask, str, NULL);
    if (g_ftp_talkback) (*g_ftp_talkback)(wxString::Format(_("(%s) %s"), mask, str));
    va_end(argptr);
}
*/

void wxLogTraceFTP(const wxChar * str)
{
    if (g_ftp_talkback) (*g_ftp_talkback)(str);
}
...
char wxFTP::SendCommand(const wxString& command)
{
    ...
    wxLogTrace(FTP_TRACE_MASK, _T("==> %s"), cmd.c_str()); // TRK
    wxLogTraceFTP(cmd);
#endif // __WXDEBUG__
    ...
    while ( !endOfReply && !badReply )
    {
        ...
        else // line has at least 4 chars
        {
            ...
            if ( firstLine )
            {
                ...
                wxLogTrace(FTP_TRACE_MASK, _T("<== %s %s"),
                           code.c_str(), line.c_str() + LEN_CODE + 1); // TRK
                wxLogTraceFTP(line.c_str() + LEN_CODE + 1);
