       ...
       : wxFrame(NULL, wxID_ANY, title)
{
    ...
    for(int i=0; i<30; i++){
        colorpanel = new wxPanel(m_scroll, wxID_ANY, wxDefaultPosition, wxSize(-1, 40));
        colorpanel->SetBackgroundColour(wxColour(96, 114, 134));
		//colorpanel->Connect(wxEVT_MOUSEWHEEL, wxMouseEventHandler(MyFrame::OnMouseWheel), NULL, this);
        mainsizer->Add(colorpanel, 0, wxEXPAND | wxBOTTOM, 1);
    }
    
    m_scroll->SetSizer(mainsizer);
    m_scroll->FitInside();
    m_scroll->SetScrollRate(5, 5);
    mastersizer->Add(m_scroll, 1, wxEXPAND);
...
{
	m_scroll->GetEventHandler()->ProcessEvent(event);
}*/
