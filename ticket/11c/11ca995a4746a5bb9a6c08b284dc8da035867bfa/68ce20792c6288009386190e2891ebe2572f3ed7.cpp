...
//KT
class MyXmlDataObject : public wxHTMLDataObject
{
public:
	MyXmlDataObject(const wxString& xml, const wxString& format)
		: wxHTMLDataObject(xml)
	{
		SetFormat(format);
	}
};
...
//	MyXmlDataObject *xml = new MyXmlDataObject("xml...", "My Custom Format");
//	Console:
//	'My Custom Format' is not a valid UTI string.  Cannot use an invalid UTI as a type
//	returned from -writeableTypesForPasteboard: in class wxPasteBoardWriter.
#if 1
	// This crashes
	MyXmlDataObject *xml = new MyXmlDataObject("xml...", "com.example.testing.myformat");
	data.Add(xml);
#else
	// This doesn't crash
	wxHTMLDataObject *html = new wxHTMLDataObject("html...");
	data.Add(html);
#endif
	wxTextDataObject *text = new wxTextDataObject(m_strText);
	data.Add(text);
