...
{
	VARIANTARG& srcVariant = const_cast<VARIANTARG&>(oleVariant);
	
	// dereference the variant until we get to the actual data
	while ( (srcVariant.vt & VT_BYREF) && ((srcVariant.vt & VT_TYPEMASK) == VT_VARIANT) ) 
	{
		srcVariant = *(srcVariant.pvarVal);
	}

    switch (srcVariant.vt & VT_TYPEMASK)
    {
    case VT_BSTR:
        {
            wxString str(wxConvertStringFromOle(srcVariant.bstrVal));
            variant = str;
            break;
        }
    case VT_DATE:
        {
            ...
            VariantTimeToDosDateTime(srcVariant.date, & dosDate, & dosTime);
    ...
    case VT_I4:
        {
            variant = (long) srcVariant.lVal;
            break;
        }
    case VT_I2:
        {
            variant = (long) srcVariant.iVal;
            break;
        }
    ...
    case VT_BOOL:
        {
            ...
            variant = (long) (srcVariant.bool != 0);
#else
            variant = (bool) (srcVariant.bool != 0);
#endif
#else
#ifndef HAVE_BOOL // Can't use bool operator if no native bool type
            variant = (long) (srcVariant.boolVal != 0);
#else
            variant = (bool) (srcVariant.boolVal != 0);
#endif
#endif
            break;
        }
    ...
    // case VT_ARRAY: // This is masked out by VT_TYPEMASK
        {
            ...
            for (cDims = 0, cElements = 1;
                cDims < srcVariant.parray->cDims; cDims ++)
                    cElements *= srcVariant.parray->rgsabound[cDims].cElements;

            // Get a pointer to the data
            HRESULT hr = SafeArrayAccessData(srcVariant.parray, (void HUGEP* FAR*) & pvdata);
            if (hr != NOERROR)
                return false;
            // Iterate the data.
            ...
            SafeArrayUnaccessData(srcVariant.parray);
            break;
        }
    case VT_DISPATCH:
        {
            variant = (void*) srcVariant.pdispVal;
            break;
        }
    case VT_NULL:
        {
