...
class CTestSocket : public wxThread
{
public:
    CTestSocket(const wxString& inURL): mURL(inURL) {}
    void*       Entry();
private:
    wxString    mURL;
};
...
void* CTestSocket::Entry()
{
    // Parse the URL
    wxURL url(mURL);
    if ( url.GetError() != wxURL_NOERR )
    {
        wxLogError("Failed to parse URL \"%s\"", mURL);
        return NULL;
    }
    
    // Try to get the input stream (connects to the given URL)
    wxLogMessage("Establishing connection to \"%s\"...", mURL);
    const std::auto_ptr<wxInputStream> data(url.GetInputStream());
    if ( !data.get() )
    {
        wxLogError("Failed to retrieve URL \"%s\"", mURL);
        return NULL;
    }
...
#if 1
    CTestSocket*    myTest = new CTestSocket(urlname);
    myTest->Create();
    myTest->Run();
#else
