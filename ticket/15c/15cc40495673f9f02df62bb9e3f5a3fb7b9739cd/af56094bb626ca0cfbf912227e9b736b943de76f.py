#!/usr/bin/env python

import wx
from wx.lib.expando import ExpandoTextCtrl
import wx.lib.inspection
import wx.lib.mixins.inspection
import sys, os

# stuff for debugging
print("Python %s" % sys.version)
print("wx.version: %s" % wx.version())
#print("pid: %s" % os.getpid()); input("Press Enter...")

assertMode = wx.APP_ASSERT_DIALOG

class P(wx.Panel):

	def __init__(self, *args, **kwargs):
		super().__init__(*args, **kwargs)
		self.mainSizer = wx.BoxSizer(wx.VERTICAL)
		self.SetSizer(self.mainSizer)
		self._setContent(self.mainSizer)

	def _setContent(self, settingsSizer: wx.BoxSizer):
		displaySizer = wx.StaticBoxSizer(wx.HORIZONTAL, self, label="static box")
		displayBox = displaySizer.GetStaticBox()
		self.displayNameCtrl = ExpandoTextCtrl(
			displayBox,
			size=(250, -1),
			style=wx.TE_READONLY
		)
		self.displayNameCtrl.SetValue("fooBarLater")
		displaySizer.Add(self.displayNameCtrl)
		settingsSizer.Add(displaySizer)

class TestDialog(wx.Dialog):
	def __init__(
			self, parent, id, title, log, size=wx.DefaultSize, pos=wx.DefaultPosition,
			style=wx.DEFAULT_DIALOG_STYLE, name='dialog',
	):
		self.log = log

		wx.Dialog.__init__(self, parent, id, title, pos, size, style, name)

		sizer = wx.BoxSizer(wx.VERTICAL)

		self.p = P(self)
		sizer.Add(self.p)

		btnsizer = wx.StdDialogButtonSizer()

		btn = wx.Button(self, wx.ID_OK)
		btnsizer.AddButton(btn)
		btn.Bind(wx.EVT_BUTTON, self.onOk)

		btnsizer.Realize()

		sizer.Add(btnsizer, 0, wx.ALL, 5)
		self.SetSizer(sizer)
		sizer.Fit(self)

	def onOk(self, evt):
		self.log.WriteText("You pressed OK\n")
		try:
			self.DestroyChildren()
			self.Destroy()
		except Exception as e:
			self.log.WriteText(f"Exception: {e}")


# Extracted from wxPython Demos
# ---------------------------------------------------------------------------

def runTest(frame, nb, log):
	dlg = TestDialog(
		nb, -1, "Sample Dialog", log, size=(350, 200),
		style=wx.DEFAULT_DIALOG_STYLE,
	)
	dlg.CenterOnScreen()
	dlg.Show()
	return dlg

# ----------------------------------------------------------------------------

class Log:
	def WriteText(self, text):
		if text[-1:] == '\n':
			text = text[:-1]
		wx.LogMessage(text)

	write = WriteText


class RunDemoApp(wx.App, wx.lib.mixins.inspection.InspectionMixin):
	def __init__(self, name, test, useShell):
		self.name = name
		self._runTest = test
		self.useShell = useShell
		wx.App.__init__(self, redirect=False)

	def OnInit(self):
		wx.Log.SetActiveTarget(wx.LogStderr())

		self.SetAssertMode(assertMode)
		self.InitInspection()  # for the InspectionMixin base class

		frame = wx.Frame(None, -1, "RunDemo: " + self.name, size=(200, 100),
			style=wx.DEFAULT_FRAME_STYLE, name="run a sample")
		frame.CreateStatusBar()

		menuBar = wx.MenuBar()
		menu = wx.Menu()
		item = menu.Append(-1, "&Widget Inspector\tF6", "Show the wxPython Widget Inspection Tool")
		self.Bind(wx.EVT_MENU, self.OnWidgetInspector, item)
		item = menu.Append(wx.ID_EXIT, "E&xit\tCtrl-Q", "Exit demo")
		self.Bind(wx.EVT_MENU, self.OnExitApp, item)
		menuBar.Append(menu, "&File")

		ns = {}
		ns['wx'] = wx
		ns['app'] = self
		ns['frame'] = frame

		frame.SetMenuBar(menuBar)
		frame.Show(True)
		frame.Bind(wx.EVT_CLOSE, self.OnCloseFrame)

		win = self._runTest(frame, frame, Log())

		# a window will be returned if the demo does not create
		# its own top-level window
		if win:
			# so set the frame to a good size for showing stuff
			frame.SetSize((800, 600))
			win.SetFocus()
			self.window = win
			ns['win'] = win
			frect = frame.GetRect()

		else:
			# It was probably a dialog or something that is already
			# gone, so we're done.
			frame.Destroy()
			return True

		self.SetTopWindow(frame)
		self.frame = frame
		# wx.Log.SetActiveTarget(wx.LogStderr())
		# wx.Log.SetTraceMask(wx.TraceMessages)

		if self.useShell:
			# Make a PyShell window, and position it below our test window
			from wx import py
			shell = py.shell.ShellFrame(None, locals=ns)
			frect.OffsetXY(0, frect.height)
			frect.height = 400
			shell.SetRect(frect)
			shell.Show()

			# Hook the close event of the test window so that we close
			# the shell at the same time
			def CloseShell(evt):
				if shell:
					shell.Close()
				evt.Skip()

			frame.Bind(wx.EVT_CLOSE, CloseShell)

		return True

	def OnExitApp(self, evt):
		self.frame.Close(True)

	def OnCloseFrame(self, evt):
		if hasattr(self, "window") and hasattr(self.window, "ShutdownDemo"):
			self.window.ShutdownDemo()
		evt.Skip()

	def OnWidgetInspector(self, evt):
		wx.lib.inspection.InspectionTool().Show()

# ----------------------------------------------------------------------------


def main(argv):
	# ensure the CWD is the demo folder
	demoFolder = os.path.realpath(os.path.dirname(__file__))
	os.chdir(demoFolder)

	sys.path.insert(0, os.path.join(demoFolder, 'agw'))
	sys.path.insert(0, '.')

	app = RunDemoApp(name="wxAssertion", test=runTest, useShell=False)
	app.MainLoop()


if __name__ == '__main__':
	main(['', os.path.basename(sys.argv[0])] + sys.argv[1:])
