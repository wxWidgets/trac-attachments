...
// Modified by:
// Created:     04/01/98
// RCS-ID:      $Id: utilsexc.cpp,v 1.87 2006/08/31 19:31:09 ABX Exp $
...
long wxExecute(const wxString& cmd, int flags, wxProcess *handler)
{
	...
	//to support commands with spaces 
	//but they MUST be encapsulated by quotes
	wxString moduleName, arguments;
	int pos = command.find( wxT('"'), 1 );
	if (command[0]==wxT('"') && pos!=wxString::npos && pos!=command.Length()-1)
	{
		moduleName = command.Mid(1, pos-1);
		arguments = command.Mid(pos+1).Trim();
	}
	else
	{
		// we are assuming commands without spaces for now
		moduleName = command.BeforeFirst(wxT(' '));
		arguments = command.AfterFirst(wxT(' '));
	}
#endif
