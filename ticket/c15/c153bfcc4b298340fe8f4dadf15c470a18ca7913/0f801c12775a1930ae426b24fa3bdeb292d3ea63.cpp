...
public:
    ...
    {
		// WNS caused by doing Clear(), then ShowHeader(false) which calls this routine
		// even though there is currently no selected page. actual crash is lower down, a knock on.
		if (page == NULL)
			return;
        m_page = page;
        OnPageUpdated();
...
void wxPropertyGridManager::Clear()
{
	...
	// WNS no pages -> no header either
	// if left enabled, a following addpage will crash trying to Realize the toolbar
	// AND user cannot call showheader(false) cos that crashes too, tries to work on missing page
	m_showHeader = false;
