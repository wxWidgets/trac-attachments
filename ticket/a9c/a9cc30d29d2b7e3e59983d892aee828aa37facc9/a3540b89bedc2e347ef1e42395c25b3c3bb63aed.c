...
#ifdef _MT
static errcode __cdecl _tsopen_nolock ( int *,
                                int *,
                                const _TSCHAR *,
                                int,
                                int,
...
errcode __cdecl _tsopen_helper (
        const _TSCHAR *path,
        int oflag,
        int shflag,
        int pmode,
...
static errcode __cdecl _tsopen_nolock (
        int *punlock_flag,
        int *pfh,
        const _TSCHAR *path,
        int oflag,
        ...
        *punlock_flag = 1;
        ...
        if ( fileflags & FTEXT) {
            ...
            /* Check for FTEXT makes sure that _O_BINARY is not set in
               ...
               below.
            */
            _ASSERTE( (oflag & (_O_BINARY) ) == 0 && _fmode != _O_BINARY);
            ...
            if ( (oflag & (_O_TEXT | _O_WTEXT | _O_U16TEXT | _O_U8TEXT) ) == 0 )
            {
                ...
                if ( (_fmode & (_O_TEXT | _O_WTEXT | _O_U16TEXT | _O_U8TEXT) ) == 0)
                    oflag |= _O_TEXT; /* default to ANSI */
                else
                    oflag |= _fmode & (_O_TEXT | _O_WTEXT | _O_U16TEXT | _O_U8TEXT);
            }
...
/***
*int _sopen_s(path, oflag, shflag, pmode) - open a file with sharing
*
*Purpose:
        ...
        )
{
    ...
    /* Last parameter passed as 0 because we want to validate
