# BUG DEMO - image in summenu on second start
# OS: WindowsXP, SP2, Classic theme
# wxPython: wx-2.7.2-msw-ansi
import wx
#------------------------------------------------------------------------------ 
def GetData():
    return \
...
class MenuFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, 'Menu image bug-1', size=(400, 200))
        p=wx.Panel(self)

        menuBar = wx.MenuBar()

        menu5 = wx.Menu()
        item = wx.MenuItem(menu5, 500, "With image")
        item.SetBitmap(GetBitmap())
        menu5.AppendItem(item)

        item = wx.MenuItem(menu5, 501, "No image")
        menu5.AppendItem(item)

        menu6 = wx.Menu()
        menu6.Append(601, "Submenu Item")
        item=menu5.AppendMenu(504, "submenu", menu6)
        item.SetBitmap(GetBitmap())

        menuBar.Append(menu5, "OpenThis")
        
        self.SetMenuBar(menuBar)
        
        wx.StaticText(p,-1,
                      "Open menu: no image for submenu\nClose this form, open again - yes image...",
                      pos=(0,50))
#------------------------------------------------------------------------------ 
class MainFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, 'Menu image bug', size=(400, 200))
        p=wx.Panel(self)
        self.b=wx.Button(p,-1,"First start")
        self.Bind(wx.EVT_BUTTON, self.OnClick, self.b)
        
    def OnClick(self, event):
        self.b.SetLabel("Second start")
        frame = MenuFrame()
        frame.Show()
#------------------------------------------------------------------------------ 
app = wx.App(False)
