...
{
    OnDeleteAll(event);
    m_treeCtrl->AddTestItemsToTree(1823, 1);
}
...
void MyTreeCtrl::AddTestItemsToTree(size_t numChildren,
                                    size_t depth)
{
    int image = -1;//wxGetApp().ShowImages() ? MyTreeCtrl::TreeCtrlIcon_Folder : -1;
    wxTreeItemId rootId = AddRoot(wxT("Root"),
                                  image, image,
                                  NULL);//new MyTreeItemData(wxT("Root item")));
    if ( image != -1 )
    {
