...
//              (demonstrates file Dialog with extra controls OSX problem)
// Author:      Julian Smart
// Modified by:
// Created:     04/01/98
       ...
       : wxFrame(NULL, wxID_ANY, title)
{
    ...
    fileMenu->Append(Minimal_Open, "&Open\tAlt-O", "Open a file dialog");
    ...
    // Create a single reusable FileDialog for this frame
    m_dialog = new wxFileDialog(this, "Open", wxEmptyString, wxEmptyString,
                                wxEmptyString, wxFD_OPEN);
    m_dialog->SetWildcard(wxEmptyString);
    // Add Extra controls to standard wxFileDialog
    //m_dialog->SetExtraControlCreator(&createMyExtraPanel);
    m_dialog->CentreOnParent();
    m_dialog->SetDirectory(wxGetHomeDir());
...
void MyFrame::OnOpen(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        wxString info;
        wxWindow * const extra = m_dialog->GetExtraControl();
        info.Printf(wxT("Full file name: %s\n")
                    wxT("Path: %s\n")
                    wxT("Name: %s\n")
                    wxT("Custom window: %s"),
                    m_dialog->GetPath().c_str(),
                    m_dialog->GetDirectory().c_str(),
                    m_dialog->GetFilename().c_str(),
    ...
    wxMessageDialog msg(this,"About to ShowModal() again and it will Assert");
    msg.ShowModal();
