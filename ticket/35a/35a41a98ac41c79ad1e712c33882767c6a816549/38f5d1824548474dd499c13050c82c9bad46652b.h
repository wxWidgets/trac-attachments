...
// Modified by:
// Created:     01/02/97
// RCS-ID:      $Id: event.h 47850 2007-08-01 15:34:48Z JS $
...
BEGIN_DECLARE_EVENT_TYPES()
    ...
    DECLARE_EVENT_TYPE(wxEVT_DETAILED_HELP, 1051)
        ...
        // Event for logging (thread safe)
    DECLARE_EVENT_TYPE(wxEVT_LOG, 1100)
...
// for thread safe logging - a wxLogEvent
// ----------------------------------------------------------------------------
class WXDLLIMPEXP_BASE wxLogEvent : public wxEvent
{
public:
    wxLogEvent( wxEventType type, int level, const wxChar *szLogMsg, time_t t);
    ...
    virtual wxEvent *Clone() const { return new wxLogEvent(*this); }

public:
	// Keep it simple, no accessors, only used internally
    int               m_logLevel;
    //wxLogLevel        m_logLevel;
    wxString          m_logMsg;     	// String to log
    time_t            m_t;              // Logging time

private:
    //DECLARE_DYNAMIC_CLASS_NO_ASSIGN(wxLogEvent)
};
...
/*
 ...
 wxEVT_DESTROY
 */
 // Moved outside #if wxUSE_GUI scope 
...
class wxLogEvent;
typedef void (wxEvtHandler::*wxLogEventFunction)(wxLogEvent&);
...
#define wxLogEventHandler(func) \
    (wxObjectEventFunction)(wxEventFunction)wxStaticCastEvent(wxLogEventFunction, &func)
...
// Log event
#define EVT_LOG(func) wx__DECLARE_EVT0(wxEVT_LOG, wxLogEventHandler(func))
