...
// Modified by:
// Created:     01/02/97
// RCS-ID:      $Id: event.cpp 47850 2007-08-01 15:34:48Z JS $
...
// Log event (MT version)
DEFINE_EVENT_TYPE(wxEVT_LOG)
...
wxEvtHandler::~wxEvtHandler()
{
    ...
    if (m_dynamicEvents)
    {
              ...
              ++it )
        {
            ...
            {
                // We have the sink (event target) role here
...
{
    // If adding a destructor notification, see if we already have one and increase ref count 
    ...
    // Subscribe to wxEVT_DESTROY_EVTHANDLER events 
...
void wxEvtHandler::OnSinkDestroyed( wxEvent& evt )
{
    ...
    // Remove all connections we had to this sink 
