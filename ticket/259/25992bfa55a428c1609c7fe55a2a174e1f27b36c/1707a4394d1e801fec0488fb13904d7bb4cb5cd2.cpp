...
void MyFrame::OnToggleAnotherToolbar(wxCommandEvent& WXUNUSED(event))
{
    ...
    else
    {
        ...
        // Remove the radio tools and re-add them
        wxToolBarToolBase *tool1 = m_tbar->RemoveTool(IDM_TOOLBAR_OTHER_1);
        wxToolBarToolBase *tool2 = m_tbar->RemoveTool(IDM_TOOLBAR_OTHER_2);
        wxToolBarToolBase *tool3 = m_tbar->RemoveTool(IDM_TOOLBAR_OTHER_3);
        m_tbar->InsertTool(0, tool1);
        m_tbar->InsertTool(1, tool2);
        m_tbar->InsertTool(2, tool3);
        m_tbar->Realize();
    }
