...
// Modified by:
// Created:     07.04.02
// RCS-ID:      $Id: keyboard.cpp 47806 2007-07-29 01:25:40Z VZ $
...
public:
    ...
    void OnKeyEmul(wxCommandEvent& event);
...
{
public:
    TextWindow(wxWindow *parent,wxTextCtrl *tt)
...
BEGIN_EVENT_TABLE(MyFrame, wxFrame)
    ...
    EVT_MENU(Keyboard_Emul, MyFrame::OnKeyEmul)
         ...
         m_winText(NULL)
{
    ...
    menuKeys->AppendCheckItem(Keyboard_Emul, _T("Emulate russian key 'д'"));
    ...
    emulate_test = new wxTextCtrl(this,wxID_ANY,_T("Emmulate me"),wxPoint(1,500));
    m_winText = new TextWindow(this,emulate_test);
...
void MyFrame::OnKeyEmul(wxCommandEvent& WXUNUSED(event))
{
    wxKeyEvent ev;
    #if wxUSE_UNICODE
        // This contains the full Unicode character
	    // in a character events in Unicode mode
	        ev.m_uniChar=1076; //russian key 'd' or 'д'
		#endif
    
    ev.m_keyCode=0; // UTF-8 ?
    
    emulate_test->EmulateKeyPress(ev);
...
void MyFrame::OnSize(wxSizeEvent& WXUNUSED(event))
{
    if ( m_winText )
    {
        ...
        emulate_test->SetSize(0, 51, size.x, 50);
        m_lboxLog->SetSize(0, 101, size.x, size.y - 100);
    }
}
