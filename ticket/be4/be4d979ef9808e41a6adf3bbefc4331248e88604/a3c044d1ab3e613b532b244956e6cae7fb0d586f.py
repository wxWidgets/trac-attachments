...
class DynamicButtonPanel(wxPanel):
	...
	def AddButton( self, Caption, event, szToolTip=None, sizerTup=(0,wxEXPAND|wxEAST|wxWEST,5) ,**wxButtonArgs):
		...
		ps = self.parent.GetSizer()
		minsize = self.btnSizer.GetMinSize()
		ps.SetItemMinSize(self, minsize.width, minsize.height)
...
class TestFrame2(wxFrame):
	def __init__(self):
		...
		self.btnPanel = None #DynamicButtonPanel(self)
#		self.btnPanel.SetBackgroundColour(wxBLUE)
	...
	# Add a button graph below the graph
	# Returns the Button Panel object
	def AddButtonPanel(self):
		self.btnPanel = DynamicButtonPanel(self)
		sizer = self.GetSizer()
		sizer.Add( self.btnPanel, 0, wxEXPAND)

		self.Layout()

		# WHAT IF PARENT IF FRAME WITHOUT A SIZER???? 
		# WHAT IF THIS METHOD IS CALLED BEFORE SIZER IS SET???
		# AS THE CASE IS HERE -- THERE IS NO PARENT -- THIS IS A FRAME
#		ps = self.GetSizer()
#		minsize = self.btnPanel.GetSizer().GetMinSize()
#		ps.SetItemMinSize(self, minsize.width, minsize.height)


		self.AddButton = self.btnPanel.AddButton
...
frame.AddButton("Test", foo)
frame.AddButton("Test2", foo)
frame.AddButton("Test3", foo)
frame.AddButtonPanel()
frame.AddButton('test4',foo)

# THIS IS THE ONLY WAY I CAN GET THE LAST PANEL TO SHOW UP
frame.AddButtonPanel()
