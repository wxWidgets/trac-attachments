...
// RCS-ID:      $Id: image.cpp,v 1.239 2006/10/30 20:03:33 VZ Exp $
...
void wxImage::Paste( const wxImage &image, int x, int y )
{
	...
	// Copy the alpha channel over
	if (image.GetAlpha())
	{
		InitAlpha();

        unsigned char* source_alpha = image.GetAlpha() + xx + yy*image.GetWidth();
        ...
        for (int j = 0; j < height; j++)
        {
            ...
            {
                if (source_alpha[i] != target_alpha[i])
                {
                    target_alpha[i] = source_alpha[i];
                }
            }
            source_alpha += source_step;
            target_alpha += target_step;
        }
	}
