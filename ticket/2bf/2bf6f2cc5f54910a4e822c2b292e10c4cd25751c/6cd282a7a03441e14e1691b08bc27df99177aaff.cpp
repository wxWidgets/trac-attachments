...
//AKT
static wxString lastdrop = wxString();
static wxString lastopen = wxString();
static void CompareTwoStrings()
{
    wxLogStatus(wxT("lastdrop=%s"), lastdrop);
    wxLogStatus(wxT("lastopen=%s"), lastopen);
    if (lastdrop == lastopen) {
       wxLogStatus(wxT("strings match"));
    } else {
       wxLogStatus(wxT("strings do NOT match! (also see Console log)"));
       
       fprintf(stderr, "lastdrop=%s\n", (const char*)lastdrop.mb_str(wxConvLocal));
       fprintf(stderr, "lastopen=%s\n", (const char*)lastopen.mb_str(wxConvLocal));
       fflush(stderr);
       
       wxString sdrop = wxString();
       for (size_t i=0; i < lastdrop.length(); i++)
           sdrop.Append( wxString::Format(wxT("%d "), (int)lastdrop[i]) );
       wxLogStatus(wxT("lastdrop chars=%s len=%d"), sdrop, (int)lastdrop.length());
       
       wxString sopen = wxString();
       for (size_t i=0; i < lastopen.length(); i++)
           sopen.Append( wxString::Format(wxT("%d "), (int)lastopen[i]) );
       wxLogStatus(wxT("lastopen chars=%s len=%d"), sopen, (int)lastopen.length());
    }
}
...
void DnDFrame::OnAbout(wxCommandEvent& WXUNUSED(event))
{
    ...
    */
    //AKT
    wxFileDialog opendlg(this, _("Choose a file"),
                        wxEmptyString, wxEmptyString, _("All files (*)|*"),
                        wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    if ( opendlg.ShowModal() == wxID_OK ) {
        lastopen = opendlg.GetPath();   // this string is bad? (fprintf output is empty)
        
        /* convert to fix??? didn't work
        wxCharBuffer buf = lastopen.fn_str();
        lastopen = wxString(buf);
        */
...
bool DnDFile::OnDropFiles(wxCoord, wxCoord, const wxArrayString& filenames)
{
    ...
    //AKT - this string seems correct (fprintf output is ok)
    lastdrop = filenames[0];
