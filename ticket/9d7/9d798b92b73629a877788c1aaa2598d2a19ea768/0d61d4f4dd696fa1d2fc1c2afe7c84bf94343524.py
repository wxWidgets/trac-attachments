...
demonstrate_goodhack = True  # Demonstrate the good Windows hack (Tab/Shift-Tab accelerators)
...
class TabEvent():
	...
	def ControlDown(self): return self.m & MOD_CONTROL
...
class MainFrame(Frame):
	def __init__(self, app):
		...
		if Platform in ['__WXMSW__', '__WXGTK__'] and demonstrate_goodhack:
...
class EditEmployeeScreen(ScrolledPanel):
	...
	def __init__(self, parent, id=-1, pos=DefaultPosition, size=DefaultSize, style=0, name='scrolledpanel'):
		...
		for w in self.tab_order:
			...
			if Platform in ['__WXMSW__', '__WXGTK__'] and (demonstrate_goodhack or demonstrate_badhack):
