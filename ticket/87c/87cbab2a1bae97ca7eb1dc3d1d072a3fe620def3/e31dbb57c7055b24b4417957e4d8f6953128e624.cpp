...
// Maintainer:  Wyo
// Created:     2003-09-01
// RCS-ID:      $Id: edit.cpp,v 1.7 2004/12/08 17:08:56 ABX Exp $
// Copyright:   (c) wxGuide
...
*/
    const int STYLE_FLAG = 7; // bold, italic & underline
	int pos = GetCurrentPos();  // get current position
	StartStyling(pos-1, 255);  // start styling one char before
	SetStyling(1, STYLE_FLAG); // style one char, means this char
...
bool Edit::InitializePrefs (const wxString &name) {
	...
	wxFont font (12, wxSWISS, wxNORMAL, wxNORMAL);
	for (int i = 0; i < 8; i++){
		int bold = (i >> 2) & 1;
		int italic = (i >> 1) & 1;
		int underline = i & 1;
		
		font.SetWeight(bold ? wxBOLD : wxNORMAL);
		font.SetStyle(italic ? wxITALIC : wxNORMAL);
		font.SetUnderlined(underline == 1);
