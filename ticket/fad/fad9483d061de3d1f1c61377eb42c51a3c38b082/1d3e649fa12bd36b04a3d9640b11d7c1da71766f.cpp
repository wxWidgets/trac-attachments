...
wxLEDNumberCtrl::wxLEDNumberCtrl(wxWindow *pParent, wxWindowID Id, wxPoint const &Pos, wxSize const &Size)
:	m_Alignment(wxLED_ALIGN_LEFT),
    m_LineMargin(-1),
    m_DigitMargin(-1),
    m_LineLength(-1),
...
bool wxLEDNumberCtrl::Create(wxWindow *pParent, wxWindowID Id, wxPoint const &Pos, wxSize const &Size)
{
    bool RetVal = wxControl::Create(pParent, Id, Pos, Size);
...
void wxLEDNumberCtrl::SetValue(wxString const &Value, bool Redraw)
{
    if (Value != m_Value)
    {
        ...
        {
            long LongVal;
            wxASSERT_MSG(Value.ToLong(&LongVal), _("wxLEDNumberCtrl can only display numeric string values."));
        }
...
void wxLEDNumberCtrl::RecalcInternals(wxSize const &CurrentSize)
{
