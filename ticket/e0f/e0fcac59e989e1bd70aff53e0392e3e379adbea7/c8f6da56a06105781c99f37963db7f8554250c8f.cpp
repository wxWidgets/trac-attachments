...
// Created:     1997
// RCS-ID:      $Id: treectrl.cpp,v 1.246 2006/11/12 17:58:27 JS Exp $
        ...
        switch ( nMsg )
        {
            case WM_LBUTTONDOWN:
                if ( htItem && isMultiple && (tvht.flags & TVHT_ONITEM) != 0 )
                {
                    ...
                    m_ptClick = wxPoint(point.x, point.y);
            ...
            case WM_MOUSEMOVE:
                ...
                if ( m_htClickedItem )
                {
                    int cx = abs(m_ptClick.x - point.x);
                    int cy = abs(m_ptClick.y - point.y);
                ...
                if ( m_dragImage )
                {
                    m_dragImage->Move(wxPoint(point.x, point.y));
                    if ( htItem )
                    {
