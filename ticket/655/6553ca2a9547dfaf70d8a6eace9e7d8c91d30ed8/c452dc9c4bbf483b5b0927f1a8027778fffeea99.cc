...
static inline bool IosSeekDirToWxSeekMode(std::ios_base::seekdir way, wxSeekMode& seekMode)
{
	switch (way)
	{
...
std::streambuf *wxStdInputStreamBuffer::setbuf(char *WXUNUSED(s), std::streamsize WXUNUSED(n))
{
	return NULL;
}

std::streampos wxStdInputStreamBuffer::seekoff(std::streamoff off, std::ios_base::seekdir way,
...
std::streampos wxStdInputStreamBuffer::seekpos(std::streampos sp, std::ios_base::openmode which)
{
	if (!(which & std::ios_base::in))
		return -1;
...
std::streamsize wxStdInputStreamBuffer::showmanyc()
{
...
std::streamsize wxStdInputStreamBuffer::xsgetn(char *s, std::streamsize n)
{
	...
	return (std::streamsize) m_stream.LastRead();
}

int wxStdInputStreamBuffer::underflow()
{
...
int wxStdInputStreamBuffer::uflow()
{
	int ch = m_stream.GetC();
...
int wxStdInputStreamBuffer::pbackfail(int c)
{
	if (c == EOF) {
		if (m_lastChar == EOF)
			return EOF;
...
std::streambuf *wxStdOutputStreamBuffer::setbuf(char *WXUNUSED(s), std::streamsize WXUNUSED(n))
{
	return NULL;
}

std::streampos wxStdOutputStreamBuffer::seekoff(std::streamoff off, std::ios_base::seekdir way,
	std::ios_base::openmode which)
{
	wxSeekMode seekMode;
...
std::streampos wxStdOutputStreamBuffer::seekpos(std::streampos sp, std::ios_base::openmode which)
{
	if (!(which & std::ios_base::out))
		return -1;
...
std::streamsize wxStdOutputStreamBuffer::xsputn(const char *s, std::streamsize n)
{
	m_stream.Write((const void *) s, (size_t) n);
	return (std::streamsize) m_stream.LastWrite();
}

int wxStdOutputStreamBuffer::overflow(int c)
{
	m_stream.PutC(c);
