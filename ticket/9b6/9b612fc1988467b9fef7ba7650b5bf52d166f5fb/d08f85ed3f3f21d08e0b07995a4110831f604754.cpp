...
// Modified by:
// Created:     04.02.00
// RCS-ID:      $Id: statbar.cpp,v 1.25 2004/10/08 19:51:30 ABX Exp $
...
class test_window : public wxWindow
{
public:
	test_window(wxWindow * parent)
		: wxWindow(parent,wxID_ANY,wxPoint(10,10),wxSize(100,100),
				   wxSIMPLE_BORDER | wxVSCROLL | wxHSCROLL | wxALWAYS_SHOW_SB)
		{
			SetScrollbar(wxVERTICAL,0,10,100,true);
			SetScrollbar(wxHORIZONTAL,0,100,1000,true);
		};
	void onScrollEvent(wxScrollWinEvent & e)
		{
			int orientation  = e.GetOrientation();
			WXTYPE eventType = e.GetEventType();
			wxChar * eventTypeName;
			
			if      (eventType == wxEVT_SCROLLWIN_TOP)				{	eventTypeName = _T("top");			}
			else if (eventType == wxEVT_SCROLLWIN_BOTTOM)			{	eventTypeName = _T("bottom");		}
			else if (eventType == wxEVT_SCROLLWIN_LINEUP)			{	eventTypeName = _T("lineup");		}
			else if (eventType == wxEVT_SCROLLWIN_LINEDOWN)			{	eventTypeName = _T("linedown");		}
			else if (eventType == wxEVT_SCROLLWIN_PAGEUP)			{	eventTypeName = _T("pageup");		}
			else if (eventType == wxEVT_SCROLLWIN_PAGEDOWN)			{	eventTypeName = _T("pagedown");		}
			else if (eventType == wxEVT_SCROLLWIN_THUMBTRACK)		{	eventTypeName = _T("thumbtrack");	}
			else if (eventType == wxEVT_SCROLLWIN_THUMBRELEASE)		{	eventTypeName = _T("thumbrelease");	}
			else													{	eventTypeName = _T("UNKNOWN");		}

			wxLogTrace(wxTRACE_Messages,_T("OnScroll: [%s][event %s] scrolled: [thumb %d][range %d][page %d]"),
					   ((e.GetOrientation()==wxVERTICAL) ? _T("Vertical") : _T("Horizontal")),
					   eventTypeName,
...
BEGIN_EVENT_TABLE(test_window, wxWindow)
	EVT_SCROLLWIN(test_window::onScrollEvent)
END_EVENT_TABLE();
