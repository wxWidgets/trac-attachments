...
// SHAPELINE/BG Test function.
void CheckTree(const wxWindowMSW* win)
{
	const wxNotebook* nb = dynamic_cast<const wxNotebook*>(win);
	if (nb != NULL)
		nb->GetPageCount();		// Test if this crashes here.

	const wxWindowList& children = win->GetChildren();
	for (unsigned i = 0; i < children.size(); i++)
		CheckTree(children[i]);
}
...
LRESULT WXDLLEXPORT APIENTRY _EXPORT wxWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message != 4868 && message != 85 && message != 297 && message != 528 && wnd != NULL)
		CheckTree(wnd);
