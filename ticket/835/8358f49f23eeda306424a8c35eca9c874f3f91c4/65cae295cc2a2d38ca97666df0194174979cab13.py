...
class ButtonPanel(wx.Panel):
    ...
    def OnButton(self, evt):
        ...
        elif not hasRabbyt and buttonDefs[evt.GetId()][0] == 'RabbytCanvas':
            dlg = wx.MessageDialog(self,
                                   'Rabbyt and/or Pygame were not found.  You can get them at\n'
                                   'http://www.pygame.org and http://matthewmarshall.org/projects/rabbyt/',
                                   'Sorry', wx.OK | wx.ICON_WARNING)
            dlg.ShowModal()
            dlg.Destroy()
...
class RabbytCanvas(MyCanvasBase):
    def InitGL(self):
        self.ticks = 0
        ...
        w, h = self.size
        rabbyt.set_viewport((w,h))
        rabbyt.set_default_attribs()

        rabbyt.set_time(0)
        self.car = rabbyt.Sprite("bitmaps/image.png")
        #return
        ...
        self.car.alpha = rabbyt.lerp(0.0, 1.0, startt=1, endt=2)

        # Rotate the car from 0 to 360 over three seconds, then repeat.
        self.car.rot = rabbyt.lerp(0, 360, dt=3, extend="repeat")


    def OnDraw(self):
	#return # why are we looping in the OnPaint handler??
        if self.ticks > 5000:
            rabbyt.clear((0,0,1.0))
        elif self.ticks > 1000:
            rabbyt.clear((0,1.0,0))
        else:
            rabbyt.clear((1.0,0,0))
    ...
    def OnTimer(self,event):
        ...
        rabbyt.set_time(self.ticks/1000.0)
        self.Refresh(False)
