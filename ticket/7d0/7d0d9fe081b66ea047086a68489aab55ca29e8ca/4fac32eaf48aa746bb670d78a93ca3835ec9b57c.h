...
public:
    // JACS
    // How to select the a bitmap
    enum BitmapSelectionMode { BitmapSelection_NearestSize, BitmapSelection_SmallerSize, BitmapSelection_LargerSize, BitmapSelection_Scale };
    ...
    // Query the providers for icon with given ID and return it. Return
    // wxNullIcon if no provider provides it.
    static wxIcon GetScaledIcon(const wxArtID& id,
                          const wxArtClient& client = wxART_OTHER,
                          const wxSize& size = wxDefaultSize,
    ...
    // JACS new suggested API
    
    // Get a bitmap for the given device-independent size at a suitable scale for the given window.
    // Calls ProvideBitmap via CreateScaledBitmap to get the actual bitmap, and uses
    // GetAvailableBitmapCount, GetAvailableBitmap to determine the best available bitmap.
    // Provides compatibility by calling GetBitmap if ProvideBitmap is not implemented.
    static wxBitmap GetScaledBitmap(const wxArtID& id, const wxArtClient& client, const wxSize& szDIP, wxWindow* window);
...
protected:
    ...
    // ProvideBitmap should be defined by art providers that will also provide size information and will
    // do NO scaling, letting wxWidgets choose appropriate scaling unless overridden by the provider.
    virtual wxBitmap ProvideBitmap(const wxArtID& WXUNUSED(id),
                                  const wxArtClient& WXUNUSED(client),
                                  const wxSize& WXUNUSED(size))
    {
        return wxNullBitmap;
    ...
    // Enumerate unscaled available bitmaps
    // If returns 0, don't know what bitmaps are available.
    virtual size_t GetAvailableBitmapCount(const wxArtID& id, const wxArtClient& client) { return 0; }
    virtual wxSize GetAvailableBitmapSize(size_t idx, const wxArtID& id, const wxArtClient& client) { return wxDefaultSize; }

    // Called by GetScaledBitmap. Override to do application-specific scaling.
    virtual wxBitmap CreateScaledBitmap(const wxArtID& id, const wxArtClient& client, const wxSize& szDIP, wxWindow* window);
    
    // Returns the bitmap size we should retrieve after, and returns a bitmap scale for Mac only.
    virtual wxSize GetScaledBitmapSize(const wxArtID& id, const wxArtClient& client, const wxSize& szDIP, wxWindow* win, double& scale);
    
    // Allows customisation of scaling.
    virtual wxSize FromDIP(const wxSize& szDIP, wxWindow* win);
    
    // Does this bitmap size exist? Calls enumeration functions to find out.
    virtual bool BitmapSizeExists(const wxArtID& id, const wxArtClient& client, const wxSize& sz);
    
    // Finds the indexes of the nearest bitmaps with percentage difference from target
    virtual bool FindNearestBitmaps(const wxArtID& id, const wxArtClient& client, const wxSize& targetSize, int& smaller, int& smallerDiff, int& larger, int& largerDiff);
    ...
    virtual int FindIndexOfNearestSize(const wxArtID& id, const wxArtClient& client, const wxSize& targetSize);
    
    // Override to always/never/conditionally use hi-def bitmaps on Mac.
    virtual double GetBitmapScale(wxWindow *win);
    
    // Bitmap selection mode
    static BitmapSelectionMode sm_bitmapSelectionMode;
    static BitmapSelectionMode GetBitmapSelectionMode() { return sm_bitmapSelectionMode; }
    static void SetBitmapSelectionMode(BitmapSelectionMode mode) { sm_bitmapSelectionMode = mode; }

// JACS: protected so we can override functionality
protected:
