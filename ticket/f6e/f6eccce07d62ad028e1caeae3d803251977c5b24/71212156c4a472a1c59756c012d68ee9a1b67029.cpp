...
class CRadioTestApp : public wxApp
{
public:
	virtual bool OnInit();
};
...
class CRadioTestFrame : public wxFrame
{
	public:
		CRadioTestFrame(const wxString & title, const wxPoint & pos, const wxSize & size);
...
class CRadioTestDlg : public wxDialog
{
public:
	CRadioTestDlg(wxWindow * parent);
};


bool CRadioTestApp::OnInit()
{
	CRadioTestFrame * frame = new CRadioTestFrame("test", wxPoint(50,50), wxSize(450,340));
	frame->Show(TRUE);
	SetTopWindow(frame);
	return TRUE;
}

BEGIN_EVENT_TABLE(CRadioTestFrame, wxFrame)
  EVT_MENU    (wxID_OPEN, CRadioTestFrame::OnTest)
  EVT_MENU    (wxID_EXIT, CRadioTestFrame::OnQuit)
END_EVENT_TABLE()


CRadioTestFrame::CRadioTestFrame(const wxString & title, const wxPoint & pos, const wxSize & size)
...
void CRadioTestFrame::OnQuit(wxCommandEvent& WXUNUSED( event ))
{
	Close(TRUE);
}

void CRadioTestFrame::OnTest(wxCommandEvent& WXUNUSED( event ))
{
	CRadioTestDlg test(this);
	test.ShowModal();
}

CRadioTestDlg::CRadioTestDlg(wxWindow * parent) : wxDialog(parent, wxID_ANY, wxString("test"))
{
...
//	::PostMessage((HWND)GetHWND(), WM_DOTTEDFOCUS, 0x30002, 0); 
#endif
}


IMPLEMENT_APP(CRadioTestApp);
